...
function animateTo(state) {
  ...
  // animationState.
  var $top = state > 0 ? $curr : $prev;
  var $bot = state > 0 ? $next : $curr;
  console.log('top:', $top.attr('class'), 'bottom:', $bot.attr('class'));
  var s = state < 0 ? Math.abs(state) : 1 - state;

  var h = Math.sqrt(s*(1-s));
  var topAngle = Math.atan2(h, s);
  var botAngle = Math.atan2(h, 1-s);
  var topScale = Math.sqrt(s*s + h*h);
  var botScale = Math.sqrt((1-s)*(1-s) + h*h);
...
function applyTransform($el, angle, scale, origin) {
  ...
  var transform = 'scaleY({{scale}}) rotateX({{angle}}deg) translateY(0) '
